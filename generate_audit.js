...
const baseDir = "ECP_AUDITORIA_COMPLETA";
...
const archContent = `# Arquitectura Enlace Constructor Pro (ECP)
...
`;
fs.writeFileSync(path.join(baseDir, "ARQUITECTURA_ECP.md"), archContent);

// 2. MODELO_DATOS_ECP.md
const modeloContent = `# Modelo de Datos ECP
...
`;
fs.writeFileSync(path.join(baseDir, "MODELO_DATOS_ECP.md"), modeloContent);

// 7. FLUJOS_ECP.md
const flujosContent = `# Flujos Paso a Paso ECP
...
`;
fs.writeFileSync(path.join(baseDir, "FLUJOS_ECP.md"), flujosContent);

// 8. LIMITACIONES_ACTUALES.md
const limitacionesContent = `# Limitaciones Actuales y Errores Conocidos
...
`;
fs.writeFileSync(path.join(baseDir, "LIMITACIONES_ACTUALES.md"), limitacionesContent);

// 10. LISTADO_ARCHIVOS_INCLUIDOS.md
const listadoContent = `# Listado de Archivos de la Auditoría
...
`;
fs.writeFileSync(path.join(baseDir, "LISTADO_ARCHIVOS_INCLUIDOS.md"), listadoContent);

// Copiar archivos
fs.mkdirSync(path.join(baseDir, "motor_calculo"), { recursive: true });
fs.mkdirSync(path.join(baseDir, "componentes"), { recursive: true });
fs.mkdirSync(path.join(baseDir, "administracion"), { recursive: true });
fs.mkdirSync(path.join(baseDir, "exportacion"), { recursive: true });
...
copySafe("src/assets/index.js", path.join(baseDir, "motor_calculo/index.js"));
copySafe("src/assets/generador_oc_modulo.js", path.join(baseDir, "componentes/generador_oc_modulo.js"));
copySafe("src/assets/modulo_proveedores.js", path.join(baseDir, "componentes/modulo_proveedores.js"));
copySafe("src/jspdf.umd.min.js", path.join(baseDir, "exportacion/jspdf.umd.min.js"));
copySafe("src/xlsx.full.min.js", path.join(baseDir, "exportacion/xlsx.full.min.js"));
copySafe("apus.json", path.join(baseDir, "administracion/apus.json"));
copySafe("materiales.json", path.join(baseDir, "administracion/materiales.json"));
copySafe("catalog.json", path.join(baseDir, "administracion/catalog.json"));

console.log("Generación completada, procediendo a comprimir...");

// Comprimir en Windows vía powershell
execSync('powershell Compress-Archive -Path "ECP_AUDITORIA_COMPLETA" -DestinationPath "ECP_AUDITORIA_COMPLETA.zip" -Force');
console.log("Comprimido a ECP_AUDITORIA_COMPLETA.zip");
