...
// 1) Replace catalog filter/search IIFE (var f=(()=>{...})()) to support:
// - special tokens: :sinprecio / :sinunidad / :sinapu
// - always returns copies with flags: __dup/__missingPrecio/__missingUnidad/__missingApu
{
  ...
  if (start !== -1) {
    ...
    if (end !== -1) {
      ...
      const after = s.slice(end + endNeedle.length);
      const replacement =
...
// 2) Add issue chips and toggle logic (only within Partidas de Obra chip row)
{
  const needle =
    'children:[["Techo","techo"],["Puertas/Vent.","puerta"],["Bano","bano"],["Cocina","cocina"],["Humedad/Gotera","humedad"],["Pintura","pintura"],["Pisos","piso"],["Electrica","electrica"]].map(([y,P])=>e.jsx("button",{onClick:()=>w(P),style:{fontSize:10,padding:"3px 8px",borderRadius:999,border:"1px solid "+a.border,background:"var(--surface)",color:a.muted,cursor:"pointer",fontWeight:700,letterSpacing:".02em"},children:y},y))';
  const repl =
    'children:[["Techo","techo"],["Puertas/Vent.","puerta"],["Bano","bano"],["Cocina","cocina"],["Humedad/Gotera","humedad"],["Pintura","pintura"],["Pisos","piso"],["Electrica","electrica"],["Sin precio",":sinprecio"],["Sin unidad",":sinunidad"],["Sin APU",":sinapu"]].map(([y,P])=>e.jsx("button",{onClick:()=>{var A=String(B||"").trim();if(P&&P[0]===":"){var S=A?A.split(/\\s+/).filter(Boolean):[];var O=S.indexOf(P)!==-1;w((O?S.filter(U=>U!==P):S.concat([P])).join(" ").trim())}else w(P)},style:{fontSize:10,padding:"3px 8px",borderRadius:999,border:"1px solid "+a.border,background:"var(--surface)",color:a.muted,cursor:"pointer",fontWeight:700,letterSpacing:".02em"},children:y},y))';
  replaceOnce(needle, repl);
}

// 3) Badges next to description in catalog table
replaceOnce(
  'e.jsx("td",{style:c.td,children:y.desc}),e.jsx("td",{style:u(d({},c.td),{color:a.m',
  'e.jsxs("td",{style:c.td,children:[y.desc,y.__dup&&e.jsx("span",{style:{marginLeft:8,fontSize:10,padding:"2px 6px",borderRadius:8,border:"1px solid "+a.border,color:"#f59e0b",background:"#f59e0b22",fontWeight:800},children:"Dup"}),y.__missingPrecio&&e.jsx("span",{style:{marginLeft:6,fontSize:10,padding:"2px 6px",borderRadius:8,border:"1px solid "+a.border,color:"#f87171",background:"#f8717122",fontWeight:800},children:"Sin precio"}),y.__missingUnidad&&e.jsx("span",{style:{marginLeft:6,fontSize:10,padding:"2px 6px",borderRadius:8,border:"1px solid "+a.border,color:"#fb923c",background:"#fb923c22",fontWeight:800},children:"Sin unidad"}),y.__missingApu&&e.jsx("span",{style:{marginLeft:6,fontSize:10,padding:"2px 6px",borderRadius:8,border:"1px solid "+a.border,color:"#38bdf8",background:"#38bdf822",fontWeight:800},children:"Sin APU"})]}),e.jsx("td",{style:u(d({},c.td),{color:a.m'
);

// 4) Normalize on save + prevent duplicates (desc+cat) within vg
{
  const startNeedle = 'I=s?p.trim():b,D=()=>{';
  const start = s.indexOf(startNeedle);
  if (start !== -1) {
    ...
    if (end !== -1) {
      ...
      const newD =
        'I=s?p.trim():b,D=()=>{var y=String(I||"").trim(),P=String(j.desc||"").trim().replace(/\\s+/g," "),A=String(j.unidad||"").trim(),S=parseFloat(j.precio);if(!y||!P||j.precio==="")return;A=A.replace(/\\u00b2/g,"2");A=A.replace(/m\\s*\\^?\\s*2/i,"m2");var O=A.toLowerCase()==="unidad"?"unidad":A;var U=t.some(Q=>Q.cat===y&&String(Q.desc||"").trim().replace(/\\s+/g," ").toLowerCase()===P.toLowerCase()&&(g===null||Q.id!==g));if(U){r("Ya existe una partida similar en esa categoria");return}var R=isNaN(S)?0:S;g!==null?(i(t.map(Q=>Q.id===g?{id:g,cat:y,desc:P,unidad:O,precio:R}:Q)),z(null)):i([...t,{id:Math.max(0,...t.map(Q=>Q.id))+1,cat:y,desc:P,unidad:O,precio:R}]),F({desc:"",unidad:"unidad",precio:""}),m(!1),C(""))}';
      s = before + newD + after;
      changed++;
    }
  }
