...
const idxPath = 'src/assets/index.js';
let content = fs.readFileSync(idxPath, 'utf8');

const caComponent = `window.CompraAgilComponent = function CaComponent({comprasAgiles:t,setComprasAgiles:i,cfg:n,setToast:m}){
...
let jgBackup = fs.readFileSync('Jg_backup.js', 'utf8');

// Modifying Jg to include Compra Ágil
// 1. Add "compra_agil":"Compra Ágil y Licitaciones" to the map of titles
jgBackup = jgBackup.replace('documentos:"Documentos de Obra",mano_obra:"Presupuesto de Mano de Obra"}', 'documentos:"Documentos de Obra",mano_obra:"Presupuesto de Mano de Obra",compra_agil:"Compra Ágil y Licitaciones"}');

// 2. Add the routing logic for compra_agil
// Let's find: e.jsx(di,{mo:!0})
// And inject the compra_agil route right after the catalog route.
// Or just right before e.jsx(di,{mo:!0}) or Ce
const routeTarget = 'if(x==="clients")return e.jsx(hg,{clients:p,setClients:C,budgets:B,cfg:l,setToast:Q});';
const routeReplacement = routeTarget + 'if(x==="compra_agil")return ye("licitaciones")?e.jsx(window.CompraAgilComponent||(()=>e.jsx("div",{children:"Cargando..."})),{comprasAgiles:s,setComprasAgiles:m,cfg:l,setToast:Q,setPage:f,budgets:B,apus:g,materiales:j,catalog:b}):e.jsx(di,{mo:!0});';
jgBackup = jgBackup.replace(routeTarget, routeReplacement);

// 3. Add to sidebar
// The sidebar has links like: {p:"dashboard",ic:"📊",t:"Dashboard"},{p:"new",ic:"➕",t:"Nuevo Presupuesto"}
// Let's insert the compra_agil right after materials or catalog.
const sbTarget = '{p:"catalog",ic:"📂",t:"Catálogo"}';
const sbReplacement = sbTarget + ',{p:"compra_agil",ic:"🛒",t:"Compra Ágil"}';
jgBackup = jgBackup.replace(sbTarget, sbReplacement);

// Combine everything
// Content is everything up to window.CompraAgilComponent
const startIdx = content.indexOf('window.CompraAgilComponent = function CaComponent');
let safeContent = '';
if (startIdx > -1) {
  safeContent = content.substring(0, startIdx);
} else {
  // if not found, just use whole content and append (though we shouldn't have duplicate)
  safeContent = content;
}

const finalJs = safeContent + caComponent + '\n' + jgBackup + '_n.createRoot(document.getElementById("root")).render(e.jsx(Jg,{}))})();';

fs.writeFileSync(idxPath, finalJs, 'utf8');
console.log('Restored Jg and injected new CaComponent successfully.');
